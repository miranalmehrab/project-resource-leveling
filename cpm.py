...
def check_if_bp_is_true_for_all_ancestors(ancestors):
    global node_matrix
...
def get_min_ancestor_ls_value(ancestors):
    ancestor_ls_values = []
    ...
    while True:
        for node in node_matrix:
            if node['BP'] is False:
                ...
                elif (check_if_bp_is_true_for_all_ancestors(node['ancestor'])):
                    node['LF'] = int(get_min_ancestor_ls_value(node['ancestor']))
                    node['LS'] = int(node['LF']) - int(node['duration'])
                    node['BP'] = True
...
def calculate_slack_time_of_the_nodes():
    global node_matrix
    for node in node_matrix:
