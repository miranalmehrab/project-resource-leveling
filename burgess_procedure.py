...
class BurgessProcedure:
	...
	def prepare_burgess_response(self):
		...
		for node in self.node_matrix:
			if node["critical"] == False:               
				R_by_time = self.delay_activity_r[node["name"]]
				R2_by_time = self.delay_activity_r2[node["name"]]
				total_R = np.sum(R_by_time)
				break

		return {"node_matrix": self.node_matrix , "R_by_time": R_by_time, "R2_by_time": R2_by_time, 
	        "optimal_total_R": total_R, "optimal_total_R_square": self.optimal_total_R_square}
